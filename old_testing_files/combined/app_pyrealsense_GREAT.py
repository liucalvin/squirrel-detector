...
servo = None  # Initialize servo as None to avoid jitter
...
config = rs.config()
config.enable_stream(rs.stream.color, 640, 480, rs.format.bgr8, 30)  # RGB stream (1280x720)
config.enable_stream(rs.stream.depth, 640, 480, rs.format.z16, 30)    # Depth stream (1280x720)
...
@app.route('/move_left')
def move_left():
    global current_position, servo
    if servo is None:
        servo = Servo(servo_pin)  # Initialize servo only when needed
    ...
    servo.value = None  # Detach the servo to stop jitter
